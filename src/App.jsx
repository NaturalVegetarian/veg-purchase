      ...
      <div className="form-section">
        ...
        <div className="form-group">
          ...
          <div className="vendor-input-wrapper">
            <input className="form-input" type="text" value={vendorName} onChange={e => setVendorName(e.target.value)} placeholder="輸入菜商名稱，或點右側 ▼ 從清單選" />
            {vendors.length > 0 && (
              <button type="button" className="vendor-dropdown-btn" onClick={() => setShowVendorList(!showVendorList)}>
                {showVendorList ? '▲' : '▼'}
              </button>
            )}
          </div>
          {showVendorList && vendors.length > 0 && (
            <div className="vendor-dropdown">
              {vendors.map(v => (
                <div key={v.id} className="vendor-dropdown-item" onClick={() => { setVendorName(v.name); setShowVendorList(false); }}>
                  🏪 {v.name}
                </div>
              ))}
            </div>
